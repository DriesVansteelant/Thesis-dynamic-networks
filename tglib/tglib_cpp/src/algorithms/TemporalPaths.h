...
namespace tglib {
	...
	std::vector<tglib::TemporalEdge> minimum_duration_path(tglib::IncidentLists<N, E> const& tg, tglib::NodeId start, tglib::NodeId target, tglib::TimeInterval ti) {
		std::vector<Time> durations(tg.getNumberOfNodes(), inf);

		auto nodes = tg.getNodes();
		durations[start] = 0;
		...
		while (!q.empty() && num_f < tg.getNumberOfNodes()) {
			...
			auto ee = tg.getNode(cur->nid).outEdges;
			if (tg.getNode(cur->nid).outEdges.size() > 0) {
	...
	std::vector<tglib::TemporalEdge>
		minimum_duration_path_index_overflow(tglib::IncidentLists<N, E> const& tg, tglib::NodeId start, tglib::NodeId target, tglib::TimeInterval ti) {
		std::vector<Time> durations(tg.getNumberOfNodes(), inf);
		durations[start] = 0;
		...
		while (!q.empty() && num_f < tg.getNumberOfNodes()) {
			...
			for (auto& e : tg.getNode(cur->nid).outEdges) {
				//for (int i = 0; i < tg.getNode(cur->nid).outEdges.size(); i++) {
					//auto e = tg.getNode(cur->nid).outEdges[i];
				if (e.t < ti.first || e.t + e.tt > ti.second) continue;
	...
	template<typename N, typename E>
	std::vector<tglib::TemporalEdge>
		minimum_duration_path_omp(tglib::IncidentLists<N, E> const& tg, tglib::NodeId start, tglib::NodeId target, tglib::TimeInterval ti, int numThreads) {
		...
		while (!q.empty() && num_f < tg.getNumberOfNodes()) {
			...
			std::vector<std::shared_ptr<PathLabel>> new_labels;

#pragma omp parallel for shared(nodelabels, new_labels) // Sharing variables among threads
			for (int i = 0; i < tg.getNode(cur->nid).outEdges.size(); ++i) {
				auto& e = tg.getNode(cur->nid).outEdges[i];
				if (e.t < ti.first || e.t + e.tt > ti.second) continue;
				...
				if (e.t >= cur->a) {
					...
					// Loop for new labels
					for (auto& label : nodelabels[e.v]) {
						if ((label->s < lnew->s && label->a >= lnew->a) || (label->s == lnew->s && label->a > lnew->a)) {
							label->deleted = true;
						}
						else if ((label->s >= lnew->s && label->a <= lnew->a) || (label->s == lnew->s && label->a <= lnew->a)) {
							dom = true;
							break;
						}
					}
...
#pragma omp critical
						{
							durations[e.v] = std::min(durations[e.v], lnew->d);
							new_labels.push_back(lnew);
						}
					}
				}
			}

#pragma omp parallel for shared(nodelabels, q)
			for (int l = 0; l < new_labels.size(); l++) {
				auto label = new_labels[l];
#pragma omp critical
				{
					nodelabels[label->nid].push_back(label);
					q.push(label);
				}
			}
	...
	template<typename N, typename E>
	std::vector<tglib::TemporalEdge> minimum_duration_path_omp2(tglib::IncidentLists<N, E> const& tg, tglib::NodeId start, tglib::NodeId target, tglib::TimeInterval ti, int numThreads) {
